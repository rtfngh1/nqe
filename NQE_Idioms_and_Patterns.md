...
**The `@fwd/` prefix is REQUIRED when importing from the Forward Library.**
This is the most common cause of confusing import errors. The prefix rule depends on
where the *importing* query lives, not on where the import target lives:

- **Org-repo queries** (anything you author, including overrides) importing a
  Forward Library module: prefix is **required**.
  - ✅ `import "@fwd/Security/STIGs/Device Parameters";`
  - ❌ `import "Security/STIGs/Device Parameters";` — fails to resolve.
- **Forward Library files importing other Forward Library files** (i.e. what you
  see inside `queries-export.proto` or any other dump of the bundled library):
  prefix is **omitted**. These files reference siblings by bare path.
  - ✅ (inside a Forward Library file) `import "Security/STIGs/Device Parameters";`

Practical implication: **never copy an `import` line verbatim from a Forward
Library file into your own query.** If the source file is a Forward Library file
and you are pulling its bare-path imports into your own query, you must prepend
`@fwd/` to each one. This applies to STIG override files, fork-and-modify work,
and any other case where you re-use bundled library code.

If a `Security/...`, `Cisco/...`, `Interfaces/...`, etc. import errors with a
"can't resolve" or similar message, prepending `@fwd/` is the first thing to try.

**Rules:**
- Paths are absolute (starting from the root of your NQE library).
- Only `export`-marked declarations are visible to importers.
- `@fwd/...` paths reference Forward's built-in library of NQE helpers, and the
  prefix is **required** from any non-Forward-Library file (see above).
...
## 19. Note for Claude: Delivering NQE Queries with Block Patterns

**Problem:** NQE block patterns use triple-backtick delimiters (` ``` `) as their literal syntax. No Markdown code fence — regardless of language tag — can safely contain them. The inner ` ``` ` that closes a block pattern declaration will always terminate the outer fence early, breaking the rendered output and making it uncopyable from chat.

This was confirmed by testing: even a ` ```fwd-nqe ` language-tagged fence fails. The inner ` ``` ` still breaks the fence.

**Solution:** When a query contains block patterns, ALWAYS deliver it as a `.nqe` plain-text file attachment using `create_file` + `present_files`. Never attempt to render it inline in chat.

Correct workflow:
1. Write the query to `/mnt/user-data/outputs/filename.nqe` using `create_file`
2. Call `present_files` so the user gets a download link
3. Do NOT paste the query inline in the response

The `.nqe` extension has no special meaning — it is just a plain text file. The triple backticks inside it are literal characters and survive download perfectly.

**Never** attempt to render a block-pattern-containing NQE query inline in chat, even with a language tag. It will always break.
