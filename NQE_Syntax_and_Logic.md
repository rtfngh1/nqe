...
- Empty: `[] : List<Integer>` (must type-annotate empty literals)
  - **In a declaration context, prefer the prefix form on the name:** `xs : List<Integer> = [];`. The postfix form attached to the literal value (`xs = [] : List<Integer>;`) is *not* a reliable substitute in a declaration — it may parse as a type-annotated expression in some positions but raise an error here. The prefix form, documented under "Type annotations" in §13, is the only form known to work for named-binding declarations.
...
**Critical: `let`-bound comprehensions also require parentheses.**

A `foreach` used as the value of a `let` binding — not inside a `select { }` — must be wrapped in parentheses, or the parser will throw:

```
ERROR: extraneous input 'foreach' expecting {'re`', '`', '[', 'when', ...}
```
...
let allBlocks = foreach r in results foreach m in r.matches select m.blocks

// RIGHT — parentheses required
let allBlocks = (foreach r in results foreach m in r.matches select m.blocks)
```

This applies whenever a comprehension appears as a standalone expression on the right-hand side of `let`, not inside a `select { field: ... }` record literal (where the braces already bound the expression).
...
**The `@fwd/` prefix is REQUIRED when an org-repo query imports a Forward Library module.** The rule is asymmetric and depends on where the *importing* file lives:

- Org-repo queries (anything you author) → must use `@fwd/...` to reach Library modules.
- Forward Library files importing siblings → use bare paths (no prefix).

So a stock Forward Library file might contain `import "Security/STIGs/Device Parameters";`, but when you pull that import into your own query (e.g. a STIG override file), it must become `import "@fwd/Security/STIGs/Device Parameters";`. **Do not copy bare-path imports verbatim from `queries-export.proto` or any Forward Library file into your own code.** Prepend `@fwd/` to each.

If a `Security/...`, `Cisco/...`, `Interfaces/...` (etc.) import fails to resolve from your own query, the first thing to try is prepending `@fwd/`.
