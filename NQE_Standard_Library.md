...
Greatest element by key function. Returns null if empty. The `keySelector` is a
**named function** passed by reference — NQE has no inline lambda syntax, so you
must declare the selector as a top-level function and pass it by name.
```
getMetric(route) = route.metric;
// ...
maxBy(routes, getMetric)
```

### `minBy`
`minBy(collection: Bag<T>, keySelector: T -> K) : T?`
Least element by key function. As with `maxBy`, the `keySelector` is a named
function passed by reference (NQE has no inline lambda syntax).
...
Sort by a key function, ascending. The `keySelector` is a **named function**
passed by reference — NQE has no inline lambda syntax, so declare the selector as
a top-level function and pass it by name.
```
getOsVersion(device) = device.platform.osVersion;
// ...
orderBy(devices, getOsVersion)
```
